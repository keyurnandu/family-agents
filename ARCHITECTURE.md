...
| **Anti-hallucination constraints** | Both the synthesis system prompt (Aria) and agent system prompts explicitly ban mentioning `.claude` folders, `settings.json`, allow lists, permission walls, or any form of "approve a write/permissions." Agents running via `claude --print` inherit Claude's training knowledge about `.claude/settings.json` permissions and hallucinate about it when they see FILE REJECTED or BASH BLOCKED outcomes — these constraints override that training |
