...
| **Auto skill consolidation** | On the first `process()` call each session, `_consolidate_skills_if_needed()` checks every active agent's `auto-learned.md`. If a file exceeds `_CONSOLIDATION_THRESHOLD` (3,000 chars) and has ≥10 lessons, Haiku merges redundant lessons, drops stale/contradictory ones, and compresses to ≤30% of the original count. Original backed up to `.bak`. Runs once per session, silent on success (one-line log), graceful on failure |
| **Test suite** | 220 pytest tests covering all optimizations — all written TDD (red→green), run in <7s with zero LLM dependencies |
