...
| **Markdown artifact guard** | `_has_markdown_artifacts(path, content)` runs before every approved file write for code files (`.js`, `.jsx`, `.ts`, `.tsx`, `.py`, `.java`, etc.). Detects two classes of contamination: (1) triple-backtick fences (```` ``` ````) — agent left the markdown code-block wrapper inside the file; (2) prose instruction lines (`Now run`, `Here is`, `Make sure`, `Don't forget`, etc.) — agent wrote explanation text instead of code. Returns a human-readable description if found; the executor emits `FILE REJECTED — NOT WRITTEN TO DISK` as the outcome with explicit instructions that the file is unchanged. FILE REJECTED now triggers the auto-retry-after-lesson flow (same as BASH FAILED), so the agent immediately retries with the rejection lesson in context. Markdown/YAML/config files are never checked. Zero false positives on clean code |
