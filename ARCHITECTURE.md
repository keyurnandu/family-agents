...
| **Timeout diagnosis** | When `subprocess.run()` raises `TimeoutExpired`, the handler decodes partial `.stdout`/`.stderr` buffered before the kill, shows the last 15 lines (yellow), then calls `_diagnose_timeout()` which runs three checks in priority order: (1) **Verbose-reporter silent hang** — if `✓ src/` lines are present but no `Test Files` summary and no `❯` progress bar, a test file is silently stuck at module-load; hint lists the completed files and advises removing `--reporter=verbose` to reveal the stuck file; (2) **Default-reporter `(0 test)` hang** — `❯ filename (0 test)` in output means that specific file is stuck at module load; hint names the file and gives a 3-step isolation strategy (`vi.mock` → `--pool=forks` → `testEnvironment: node`); (3) **General interactive patterns** — watch mode, credential prompts, server startup, file watchers, slow builds/compiles. The plain-English hint is appended to both the console output and the `BASH FAILED` outcome string — so the agent can self-correct without human explanation. No-output fallback: if the process produced nothing, the hint says the process likely started in watch/interactive mode |
| **Test suite** | 310 pytest tests covering all optimizations — all written TDD (red→green), run with zero LLM dependencies |
