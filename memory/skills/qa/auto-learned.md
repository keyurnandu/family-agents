...
When all vitest test files show ✓ but the process hangs with no 'Test Files N passed' summary, the hang is in teardown — a leaked handle is keeping a worker thread's event loop alive. `forceExit: true` does NOT fix this (it applies to the main process, not worker threads). Use `pool: 'forks'` in vite.config.js — forked child processes are killed when the file completes regardless of pending handles. Also add a global `HTMLCanvasElement.prototype.getContext = vi.fn(...)` mock in setupTests.jsx for any project rendering PDF/canvas components.

### [2026-05-24 14:36] via lesson
Before running vitest integration tests with a 120s timeout, ensure all server connections and async resources are explicitly closed in afterAll hooks, or increase the timeout threshold to account for cleanup overhead.
