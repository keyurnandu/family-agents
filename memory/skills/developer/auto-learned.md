...
When `npx vitest run` times out AFTER all test files show ✓ (no 'Test Files N passed' summary), the hang is in vitest teardown — an open async handle is keeping a worker thread's event loop alive. `forceExit: true` in config does NOT fix this (it only applies to the main vitest process, not to worker threads). The correct fix is `pool: 'forks'` in vite.config.js — forked child processes are killed when the file completes regardless of pending handles. Also add a global `HTMLCanvasElement.prototype.getContext = vi.fn(...)` mock in `setupTests.jsx` to eliminate canvas-related timer leaks at their source.
...
### [2026-05-24 14:41] via lesson
Always add `--no-coverage` to `npx vitest run` commands; vitest hangs after test completion during coverage collection even after all tests pass. Consider adding a timeout or process cleanup script to catch future hangs earlier.
