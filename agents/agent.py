...
_EXEC_INSTRUCTIONS = (
    ...
    "Create / overwrite a file (use for NEW files or MAJOR rewrites):\n"
    "```\nEXEC:file:path/to/file\n```\n<file content here>\n```\n\n"
    "Edit an existing file (use for SMALL changes — saves tokens):\n"
    "```\nEXEC:edit:path/to/file\n```\n"
    "DELETE 42\n"
    "REPLACE 15: new line content here\n"
    "INSERT 10: new line to insert before line 10\n"
    "REPLACE_STRING: old text -> new text\n"
    "```\n\n"
    "Run a shell command:\n"
    "```\nEXEC:bash\n```\n<commands>\n```\n\n"
    ...
    "**PREFER EXEC:edit over EXEC:file** when changing fewer than ~20 lines in an existing file. "
    "EXEC:file requires the COMPLETE file content (expensive). "
    "EXEC:edit sends only the changed lines (cheap).\n\n"
    "Rules:\n"
    ...
    "- For EXEC:file: always include the COMPLETE file content, never a partial snippet\n"
    "- For EXEC:edit: use line numbers from READ_FILE output or REPLACE_STRING for text search\n"
    ...
    "- NEVER use bare `npm test` — it starts watch mode and gets blocked\n"
    "- Frontend (Vitest): `cd frontend && node_modules\\.bin\\vitest run --no-coverage`\n"
    "- Frontend (npm): `cd frontend && set CI=true && npm test -- --verbose`\n"
    "- Backend (Python): `cd backend && venv\\Scripts\\python.exe -m pytest tests/ -v --tb=short`\n"
    "- The system auto-fixes blocked commands when possible, but using the correct "
    "command from the start avoids wasted iterations.\n\n"
